...
import { Plus, Search, Edit2, Trash2, Package, X, Check, ImagePlus } from 'lucide-react';
...
export default function ProductsPage() {
    ...
    const unsub = onSnapshot(q, (snap) => {
      const data: Product[] = snap.docs.map((d) => {
        ...
        return {
          ...
          isFlashDeal: r.isFlashDeal ?? false,
          rating: r.rating ?? 0,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      let images: string[] = editProduct?.images ?? [];
      if (imageFile) {
        const productId = editProduct?.id ?? `p_${Date.now()}`;
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="sticky top-0 bg-white px-6 py-4 border-b border-gray-100 flex items-center justify-between rounded-t-2xl z-10">
              ...
              <button onClick={() => !saving && setShowModal(false)} className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center hover:bg-gray-200">
                <X size={16} />
              </button>
            </div>
            <div className="p-6 space-y-4">
              ...
              {/* Image Upload */}
              <div>
                <label className="text-xs font-semibold text-gray-600 mb-1.5 block">Product Image</label>
                <div
                  onClick={() => fileInputRef.current?.click()}
                  className="w-full h-40 border-2 border-dashed border-gray-200 rounded-xl flex flex-col items-center justify-center cursor-pointer hover:border-blue-400 hover:bg-blue-50/30 transition-all overflow-hidden relative">
                  {imagePreview ? (
                    <>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input value={form.discountPrice} onChange={e => setForm({ ...form, discountPrice: e.target.value })}
                    type="number" placeholder="Leave empty for no discount" className="w-full border border-gray-200 rounded-xl px-3 py-2.5 text-sm outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400" />
                </div>
                <div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button onClick={handleSave} disabled={saving || !form.name.trim() || !form.price}
                  ...
                  {saving ? (imageFile ? 'Uploading...' : 'Saving...') : (editProduct ? 'Save Changes' : 'Add Product')}
                </button>
              </div>
            </div>
          </div>
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl p-6 w-full max-w-sm text-center">
            ...
            <p className="text-sm text-gray-500 mb-6">This will remove it from Firestore and the Flutter app immediately.</p>
